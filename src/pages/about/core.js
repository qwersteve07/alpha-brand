  ...
  return (
    <section className={styles.core}>
      ...
      <div className={styles.list}>
        <Controller>
          {coreData.map((item, key) => {
            return (
              <div className={styles.item} key={item.id}>
                <Scene triggerElement={`#${item.id}`} triggerHook={0.7} duration={windowHeight / 2} reverse={true}>
                  {progress => (
                    <Tween
                      from={{
                        opacity: 0,
                      ...
                      to={{
                        opacity: 1,
                        rotateX: '0deg',
                        rotateY: key % 2 === 0 ? '180deg' : '-180deg',
                        rotateZ: '0deg',
                      }}
                      totalProgress={progress}
                      paused
                    >
                      <div id={item.id} className={styles.frame} />
                    </Tween>
                  )}
                </Scene>
